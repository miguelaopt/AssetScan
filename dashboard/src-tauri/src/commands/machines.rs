...
pub async fn get_dashboard_stats(
    ...
    let total_ram_gb = machines.iter()
...
pub async fn compare_machines(
    ...
    let machine_a = machines.iter()
        .find(|m| m.machine_id == machine_ids[0])
        .ok_or("Machine A not found")?
        .clone();
    
    let machine_b = machines.iter()
        .find(|m| m.machine_id == machine_ids[1])
        .ok_or("Machine B not found")?
        .clone();
    
    let software_a = database::get_software(&pool, &machine_ids[0])
        .map_err(|e| e.to_string())?;
    let software_b = database::get_software(&pool, &machine_ids[1])
        .map_err(|e| e.to_string())?;
...
pub async fn get_screen_time(
    ...
    database::get_screen_time(&pool, &machine_id, date.as_deref())
        .map_err(|e| e.to_string())
}
